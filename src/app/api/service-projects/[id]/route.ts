...
export async function GET(
  ...
  try {
    ...
    logDebug(`[GET /api/service-projects/${id}] Request received`)
    
    // Get the current user session
    const session = await getServerSession()
    if (!session?.user?.id) {
      logError(`[GET /api/service-projects/${id}] Authentication required`)
      return NextResponse.json({ error: 'Authentication required' }, { status: 401 })
    }

    if (!id) {
      logError(`[GET /api/service-projects/${id}] Project ID is required`)
    ...
    logDebug(`[GET /api/service-projects/${id}] User ID: ${userId}, isAdmin: ${isAdmin}`)
    ...
    logDebug(`[GET /api/service-projects/${id}] Query:`, JSON.stringify(query))
    ...
    if (projectResponse.totalDocs === 0) {
      logError(`[GET /api/service-projects/${id}] Project not found or access denied`)
    ...
    logError(`[GET /api/service-projects/${id}] Error:`, error)
