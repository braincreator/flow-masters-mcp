 ...
 * Перенаправляет запросы с /api/v1/calendly-settings на /api/calendly-settings
 */

function createRedirect(request: NextRequest) {
  const url = new URL(request.url)
  const newPath = url.pathname.replace('/api/v1/calendly-settings', '/api/calendly-settings')
