 ...
 * Перенаправляет запросы с /api/v1/calendly-webhook на /api/calendly-webhook
 */

function createRedirect(request: NextRequest) {
  const url = new URL(request.url)
  const newPath = url.pathname.replace('/api/v1/calendly-webhook', '/api/calendly-webhook')
