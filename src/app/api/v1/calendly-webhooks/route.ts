 ...
 * Перенаправляет запросы с /api/v1/calendly-webhooks на /api/calendly-webhooks
 */

function createRedirect(request: NextRequest) {
  const url = new URL(request.url)
  const newPath = url.pathname.replace('/api/v1/calendly-webhooks', '/api/calendly-webhooks')
