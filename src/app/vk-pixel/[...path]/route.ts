...
export async function GET(
  ...
  try {
    ...
    const url = `https://vk.com/${path}`
    
    console.log(`[VK Pixel Proxy] Fetching: ${url}`)
    ...
    if (!response.ok) {
      console.error(`[VK Pixel Proxy] Error: ${response.status} ${response.statusText}`)
    ...
    console.log(`[VK Pixel Proxy] Success: ${url} (${content.length} bytes)`)
    ...
    console.error('[VK Pixel Proxy] Fetch error:', error)
