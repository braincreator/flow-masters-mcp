            ...
            {milestones.map((milestone, index) => (
              ...
              >
                <div className="flex items-start">
                  <div
                    className={`absolute left-4 mt-1.5 w-3 h-3 rounded-full -translate-x-1.5 border-2 border-white dark:border-gray-900 ${
                  ...
                  <div className="ml-10">
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border dark:border-gray-700 hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex">
                          <span className={getMilestoneStatusBadge(milestone.status)}>
                            {t(`milestonesTab.status.${milestone.status}`, {
                              defaultValue: milestone.status,
                            })}
                          </span>
                          <span className={getMilestonePriorityBadge(milestone.priority)}>
                            {t(`milestonesTab.priority.${milestone.priority}`, {
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 flex flex-wrap gap-4">
                        {milestone.startDate && (
                          <span>
                            {t('milestonesTab.startDate')}: {formatDate(milestone.startDate, lang)}
                          </span>
                        )}
                        {milestone.dueDate && (
                          <span>
                            {t('milestonesTab.dueDate')}: {formatDate(milestone.dueDate, lang)}
                          </span>
                        )}
                        {milestone.completionDate && (
                          <span
                            className={`${new Date(milestone.completionDate) > new Date(milestone.dueDate || '') ? 'text-red-500 dark:text-red-400' : 'text-green-500 dark:text-green-400'}`}
                          >
                            {t('milestonesTab.completionDate')}:{' '}
                            {formatDate(milestone.completionDate, lang)}
                          </span>
                        )}
                        <span>
                          {t('milestonesTab.progress')}: {milestone.progress}%
                        </span>
                      </div>
                          ...
                          <div className="mt-2">
                            <div className="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700">
                              <div
                                ...
                                style={{
                                  width: `${(milestone.associatedTasks.filter((t) => t.status === 'completed').length / milestone.associatedTasks.length) * 100}%`,
                                }}
                              ></div>
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-lg w-full">
            <h3 className="text-lg font-semibold mb-4">
              {t('milestonesTab.createMilestoneTitle')}
            </h3>
