...
export async function GET(
  ...
  try {
    ...
    const url = `https://mc.webvisor.org/${path}`
    
    console.log(`[Metrika Proxy] Fetching: ${url}`)
    ...
    if (!response.ok) {
      console.error(`[Metrika Proxy] Error: ${response.status} ${response.statusText}`)
    ...
    console.log(`[Metrika Proxy] Success: ${url} (${content.length} bytes)`)
    ...
    console.error('[Metrika Proxy] Fetch error:', error)
