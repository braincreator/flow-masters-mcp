...
export function fixJson(content: string): string {
  ...
  try {
    ...
    content = content.replace(/"title"\s*:\s*"([^"]+),\s*excerpt"\s*:/g, '"title": "$1", "excerpt":');
 ...
 * @param content Строка с потенциально некорректным JSON
 * @returns Объект JSON или null, если преобразование невозможно
 */
export function parseJsonSafely<T>(content: string): T | null {
  try {
