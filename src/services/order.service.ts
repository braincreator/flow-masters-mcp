import type { Order, Product, Course, User, Service } from '../payload-types' // Removed Payment as it's not exported
...
type OrderCreateInput = Partial<Omit<Order, 'id' | 'createdAt' | 'updatedAt'>> & {
  ...
  locale?: string // Добавляем поле для локали
  paymentData?: Record<string, any> // Добавляем поле для данных платежа
}
...
export class OrderService extends BaseService {
  ...
  async create(input: OrderCreateInput): Promise<Order | null> {
    try {
      // Получаем локаль для конвертации цены
      const locale = input.locale || 'en'
      const sourceLocaleForPrice = 'en' // Базовая валюта для цен
      ...
      // Обновляем входные данные с конвертированными ценами
      const orderData = {
        ...input,
        items: updatedItems,
        ...
        paymentData: input.paymentData
          ? Object.assign({}, input.paymentData, {
              ...
              priceConversionApplied: true,
            })
          : {
              customerLocale: locale,
              priceConversionApplied: true,
            },
        status: input.status || 'pending', // Добавляем статус для соответствия типу Order
      }

      // Создаем заказ с обновленными данными
      const order = await this.payload.create({
        collection: 'orders',
        data: orderData as any, // Используем any для обхода проверки типов
      })
      ...
      // Обновляем кеш
      if (order) {
        // Revalidate content after order creation
    ...
    return this.withErrorHandling(async () => {
      ...
      if (input.items) {
        dataToUpdate.items = input.items.map((item) => ({
          ...
          ...(item.service && {
            service: typeof item.service === 'string' ? item.service : (item.service as any).id,
          }),
        }))
      }
    ...
    try {
      ...
      if (!userId) {
        this.payload.logger.warn(
          `[OrderService] Customer ID not found for order ${order.id}. Cannot process course enrollments.`,
        )
        return
      }
      ...
      for (const item of items) {
        ...
        const productId =
          typeof productField === 'string' ? productField : (productField as Product)?.id
        ...
        if (product && product.isCourse && product.course) {
          ...
          const courseId =
            typeof courseField === 'string' ? courseField : (courseField as Course)?.id

          if (!courseId) {
            this.payload.logger.warn(
              `[OrderService] Course ID not found for product ${productId} in order ${order.id}.`,
            )
            continue
          }
          ...
          if (!courseDetails) {
            this.payload.logger.warn(
              `[OrderService] Course ${courseId} not found for order ${order.id}.`,
            )
            continue
          }
      ...
      this.payload.logger.error(
        `Error processing course enrollments for order ${order.id}: ${error.message}`,
        error,
      )
    }
  }
  ...
  async initiateCancellation(
    ...
    return this.withErrorHandling(async () => {
      this.payload.logger.info(
        `[OrderService] Initiating cancellation for orderId: ${orderId}, Reason: ${reason || 'N/A'}`,
      )
      ...
      if (
        ...
        order.status === ('refunded' as Order['status'])
      ) {
        this.payload.logger.info(
          `[OrderService] Order ${orderId} is already in a final state: ${order.status}.`,
        )
        return {
          success: true,
      ...
      if (!cancellableStatuses.includes(order.status)) {
        this.payload.logger.warn(
          `[OrderService] Order ${orderId} has status ${order.status}, which is not directly cancellable through this flow. Manual intervention may be required.`,
        )
        ...
        return {
          success: false,
          message: `Order ${orderId} in status ${order.status} cannot be automatically cancelled.`,
          status: order.status,
        }
      }

      // Attempt to void/refund payment if applicable
      if (order.paymentId && (order.status === 'processing' || order.paidAt)) {
        // Assuming 'processing' means payment was attempted/successful
        try {
          const paymentService = ServiceRegistry.getInstance(this.payload).getPaymentService()
          this.payload.logger.info(
            `[OrderService] Attempting payment void for order ${orderId}, paymentId ${order.paymentId}.`,
          )
          ...
          const paymentResult = await paymentService.voidPayment(
            orderId,
            reason || 'Order cancelled by user/system',
          )

          if (paymentResult.status === 'voided' || paymentResult.status === 'succeeded') {
            // 'succeeded' might be returned by some stubs for void
            this.payload.logger.info(
              `[OrderService] Payment void successful for order ${orderId}, paymentId ${order.paymentId}`,
            )
          } else {
            this.payload.logger.warn(
              `[OrderService] Payment void failed for order ${orderId}, paymentId ${order.paymentId}: ${paymentResult.errorMessage}. Attempting refund.`,
            )
            // Если void fails (e.g., transaction already settled), attempt a refund.
            const refundResult = await paymentService.refundPayment(
              orderId,
              'USD', // Используем USD как валюту по умолчанию
              order.total?.en?.amount, // Передаем сумму как третий параметр
              reason || 'Order cancelled - void failed', // Передаем причину как четвертый параметр
            )
            if (refundResult.status === 'refunded' || refundResult.status === 'succeeded') {
              this.payload.logger.info(
                `[OrderService] Payment refund successful for order ${orderId} after void failed.`,
              )
            } else {
              this.payload.logger.error(
                `[OrderService] Payment void AND refund failed for order ${orderId}, paymentId ${order.paymentId}: ${refundResult.errorMessage}`,
              )
          ...
          this.payload.logger.error(
            `[OrderService] Error during payment void/refund for order ${orderId}: ${paymentError.message}`,
            paymentError,
          )
          // Depending on business logic, you might want to prevent order cancellation here or flag for manual review.
        }
      ...
      await this.triggerEvent('order.cancelled', {
        ...updatedOrder,
  ...
  async initiateRefund(
    ...
    return this.withErrorHandling(async () => {
      this.payload.logger.info(
        `[OrderService] Initiating refund for orderId: ${orderId}, Amount: ${amountToRefund ?? 'Full'}, Reason: ${reason || 'N/A'}`,
      )
      ...
      if (!order.paidAt && !order.paymentId) {
        this.payload.logger.warn(
          `[OrderService] Order ${orderId} has not been paid or has no payment ID. Cannot process refund directly. Consider cancelling.`,
        )
        // If not paid, typically it would be a cancellation.
        return {
          success: false,
          message: `Order ${orderId} was not paid. Consider cancelling instead.`,
          status: order.status,
        }
      }
      ...
      if (typeof orderTotal !== 'number') {
        this.payload.logger.error(
          `[OrderService] Order ${orderId} total amount is invalid. Cannot process refund.`,
        )
      ...
      if (finalAmountToRefund <= 0) {
        this.payload.logger.warn(
          `[OrderService] Refund amount for order ${orderId} must be positive. Amount: ${finalAmountToRefund}`,
        )
        return { success: false, message: 'Refund amount must be positive.' }
      }
      if (finalAmountToRefund > orderTotal) {
        this.payload.logger.warn(
          `[OrderService] Refund amount ${finalAmountToRefund} for order ${orderId} exceeds order total ${orderTotal}. Adjusting to full refund.`,
        )
      ...
      if (order.paymentId) {
        try {
          const paymentService = ServiceRegistry.getInstance(this.payload).getPaymentService()
          this.payload.logger.info(
            `[OrderService] Attempting payment refund for order ${orderId}, paymentId ${order.paymentId}, amount ${finalAmountToRefund}.`,
          )
          const paymentResult = await paymentService.refundPayment(
            orderId, // Первый параметр - ID заказа
            'USD', // Второй параметр - валюта (USD по умолчанию)
            finalAmountToRefund, // Третий параметр - сумма возврата
            reason || 'Order refunded by user/system', // Четвертый параметр - причина возврата
          )

          if (paymentResult.status === 'refunded' || paymentResult.status === 'succeeded') {
            // 'succeeded' might be returned by some stubs
            this.payload.logger.info(
              `[OrderService] Payment refund successful for order ${orderId}, paymentId ${order.paymentId}, amount ${finalAmountToRefund}. New transaction ID (if any): ${paymentResult.paymentId}`,
            )
            paymentRefundSuccessful = true
          } else {
            this.payload.logger.error(
              `[OrderService] Payment refund failed for order ${orderId}, paymentId ${order.paymentId}: ${paymentResult.errorMessage}`,
            )
          }
        } catch (paymentError: any) {
          this.payload.logger.error(
            `[OrderService] Error during payment refund for order ${orderId}: ${paymentError.message}`,
            paymentError,
          )
        }
      } else {
        this.payload.logger.warn(
          `[OrderService] No paymentId found for order ${orderId}. Cannot process refund through payment gateway. Marking as successful for internal status update if no payment ID was present.`,
        )
      ...
      if (!paymentRefundSuccessful && order.paymentId) {
        ...
        return {
          success: false,
          message: `Payment refund failed for order ${orderId}. Order status not changed.`,
        }
      }

      const newStatus: Order['status'] =
        finalAmountToRefund === orderTotal
          ? ('refunded' as Order['status'])
          : ('partially_refunded' as Order['status'])
      ...
      await this.triggerEvent('order.refunded', {
        ...updatedOrder,
        refundAmount: finalAmountToRefund,
        refundReason: reason,
      } as Order)
