...
export interface AnalyticsEvent {
  courseId: string
  userId?: string
  eventType: 'view' | 'enrollment' | 'start' | 'completion' | 'module_completion' | 'lesson_completion' | 'rating' | 'purchase'
  moduleId?: string
  lessonId?: string
      ...
      switch (event.eventType) {
        ...
        case 'enrollment':
          ...
          if (analytics.views > 0) {
            analytics.conversionRates.landingToEnrollment = (analytics.enrollments / analytics.views) * 100
          }
          break
        ...
        case 'start':
          ...
          if (analytics.enrollments > 0) {
            analytics.conversionRates.enrollmentToStart = (event.value || 1 / analytics.enrollments) * 100
          }
          break
        ...
        case 'completion':
          ...
          if (analytics.conversionRates.enrollmentToStart > 0) {
            const startCount = (analytics.enrollments * analytics.conversionRates.enrollmentToStart) / 100
            if (startCount > 0) {
              analytics.conversionRates.startToCompletion = (analytics.completions / startCount) * 100
            }
          }
          break
        ...
        case 'module_completion':
          if (event.moduleId) {
            ...
            let moduleStats = analytics.moduleCompletionRates?.find((item: any) => item.moduleId === event.moduleId)
            ...
            if (!moduleStats && event.moduleId) {
              ...
              if (module) {
                ...
                moduleStats = analytics.moduleCompletionRates[analytics.moduleCompletionRates.length - 1]
              }
            }
        ...
        case 'lesson_completion':
          if (event.lessonId && event.moduleId) {
            ...
            let lessonStats = analytics.lessonCompletionRates?.find((item: any) => item.lessonId === event.lessonId)
            ...
            if (!lessonStats) {
              ...
              if (lesson && module) {
                ...
                lessonStats = analytics.lessonCompletionRates[analytics.lessonCompletionRates.length - 1]
              }
            }
        ...
        case 'rating':
          if (event.value !== undefined) {
            ...
            analytics.averageRating = analytics.completions > 0 ? newTotal / analytics.completions : event.value
          }
          break
