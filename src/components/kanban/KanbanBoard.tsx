...
export function KanbanBoard({
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    const sourceIndex = tasks.filter(t => t.status === sourceStatus).findIndex(t => t.id === activeTaskId)
    ...
    if (Object.keys(COLUMN_DEFINITIONS).includes(overId)) {
      ...
      destinationIndex = tasks.filter(t => t.status === destinationStatus).findIndex(t => t.id === overId)
    }
    ...
    if (destinationColumn.limit && sourceStatus !== destinationStatus) {
      ...
      if (currentTasksInDestination >= destinationColumn.limit) {
        // TODO: Show error message about WIP limit
        console.warn(`WIP limit exceeded for ${destinationStatus}`)
        return
      }
    }
