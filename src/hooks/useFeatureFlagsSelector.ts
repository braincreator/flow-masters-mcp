 ...
 */
export function useFeatureFlagsSelector<T>(selector: (context: any) => T): T {
...
export function useFeatures(featureIds: string[]) {
  return useFeatureFlagsSelector(context => {
    const result: Record<string, { isEnabled: boolean; variant: string | null; metadata: Record<string, any> | null }> = {}
...
export function useAnyFeatureEnabled(featureIds: string[]) {
  return useFeatureFlagsSelector(context => 
    featureIds.some(id => context.isFeatureEnabled(id))
  )
}
