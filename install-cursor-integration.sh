...
cp "$(dirname "$0")/docker-compose.cursor.yml" "$MCP_DIR/docker-compose.yml" 2>/dev/null || {
...
# Copy Docker config
cp "$(dirname "$0")/docker-config.json" "$MCP_DIR/config.json" 2>/dev/null || {
...
echo "MCP server started! Access it at http://localhost:3030"
EOL
...
echo "To start the MCP server, run: $MCP_DIR/start-mcp.sh"
